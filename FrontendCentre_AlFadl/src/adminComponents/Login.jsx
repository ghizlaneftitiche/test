  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.access_token) {
        ...
        else if (role === "formateurBranche") navigate("/formateurBranche");
        else if (role === "formateurModule") navigate("/formateurModule");
      }
    } catch (err) {
  ...
  return (
    <div className="bg-gray-50 min-h-screen flex items-center justify-center p-4">
      <div className="w-full max-w-[450px] min-h-[550px] flex flex-col p-8 rounded-2xl bg-white shadow-2xl">
        ...
        {error && (
          <div className="bg-red-50 text-red-600 text-sm p-3 rounded-lg text-center mb-4 border border-red-100">
            {error}
          </div>
        )}
